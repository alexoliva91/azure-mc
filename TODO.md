...
## High priority 
1. Add the ability to use 'autocorrelation time' as a convergence diagnostic for MCMC chains:
...
2. Add other R-matrix parameters even if not explicitly supported by AZURE2 (the parameters may be included in the input
...
3. Since these extra parameters are not explicitly supported by AZURE2, they will not be included in the `parameters.yaml` file created by the `azure-mc params -i input.azr` command, therefore:
...
## Medium priority
1. Replace ambiguous `populate` command:
...
2. Replace ambiguous `extrapolate` command as the name does not clearly indicate its purpose:
  - [ ] Use `azure-mc mc calculate` for performing AZURE2 `Calculation without Data` of the S-factor/Cross-Section using monte carlo sampling of the parameters.
  - [ ] Use `azure-mc mcmc calculate` for performing AZURE2 `Calculation without Data` of the S-factor/Cross-Section using the MCMC sampling of the parameters previously run with `azure-mc mcmc fit`.

3. Implement simple plot drawing functionality from *_results.npz files:
