...
|-----|---------|-------------|
| `n_walkers` | `max(2*N_params + 2, 32)` | Number of emcee walkers (must be ≥ 2 × n_params) |
...
| `max_workers` | `auto` | Parallel worker count (`auto` → `cpu_count // azure_threads`) |
| `azure_threads` | `auto` | `OMP_NUM_THREADS` per AZURE2 process (`auto` → 4/2/1 by CPU count) |
| `seed` | `42` | Random seed for reproducibility |
| `keep_tmp` | `false` | Keep temporary run directories |
| `timeout` | `0` | Per-run timeout in seconds (`0` = wait indefinitely) |
...
| `params_file` | `params.yaml` | Parameters file path |
| `quantiles` | `[0.16, 0.50, 0.84]` | Quantile levels to compute |
| `n_walkers` | `max(2*N+2, 32)` | Number of emcee walkers |
