...
Workflow — random MC (extrapolation without data):
  1. {_PROG} populate  -i input.azr
  2. (edit parameters.yaml — adjust ranges, distributions)
  3. {_PROG} mc extrapolate  -i input.azr  -c setup.yaml
  4. {_PROG} mc summary      -r mc_extrapolate.npz

Workflow — MCMC (fit to data, then extrapolate):
  1. {_PROG} populate  -i input.azr
  2. (edit parameters.yaml — adjust priors)
  3. {_PROG} mcmc fit          -i input.azr  -c setup.yaml
  4. {_PROG} mcmc extrapolate  -i input.azr  -c setup.yaml  --chain mcmc_chain.npz
...
""",
    ...
    sp = sub.add_parser("populate",
                        help="Discover free parameters → YAML config files")
    sp.add_argument("-i", "--input", required=True, dest="azr_file",
                    help="AZURE2 .azr input file")
    sp.add_argument("--setup-out", default="setup.yaml",
                    help="Output setup file (default: setup.yaml)")
    sp.add_argument("--params-out", default="parameters.yaml",
                    help="Output parameters file (default: parameters.yaml)")
    ...
    mc_ext.add_argument("-c", "--config", required=True, dest="setup",
                        help="Setup YAML (references params file)")
    ...
    mcmc_fit.add_argument("-c", "--config", required=True, dest="setup",
                          help="Setup YAML (with mcmc section)")
    ...
    if args.command == "populate":
        ...
        if args.mc_command == "extrapolate":
